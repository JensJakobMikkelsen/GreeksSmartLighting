...
class MainActivity : AppCompatActivity(), ChatFragment.CommunicationListener,
    ...
    @RequiresApi(Build.VERSION_CODES.LOLLIPOP)
    override fun itemLongClicked(deviceData: DeviceData) {
        if(connected) {
            disconnectDevice(deviceData)
        }
    }
/*
*/
    ...
    @RequiresApi(Build.VERSION_CODES.LOLLIPOP)
    private fun disconnectDevice(deviceData: DeviceData) {
        ...
        mChatService?.disconnect(device, true)
    ...
    private fun showCircularFragment() {
        if(!isFinishing) {
            val fragmentManager = supportFragmentManager
            val fragmentTransaction = fragmentManager.beginTransaction()
            circularFragment = CircularFragment.newInstance()
            fragmentTransaction.replace(R.id.mainScreen, circularFragment, "CircularFragment")
            fragmentTransaction.addToBackStack("CircularFragment")
            fragmentTransaction.commit()
        }
    }

    private fun showMenuSelectionFragment() {
        if(!isFinishing) {
            val fragmentManager = supportFragmentManager
            val fragmentTransaction = fragmentManager.beginTransaction()
            menuSelectionFragment = MenuSelectionFragment.newInstance()
            fragmentTransaction.replace(R.id.mainScreen, menuSelectionFragment, "menuSelectioNFragment")
            fragmentTransaction.addToBackStack("menuSelectioNFragment")
            fragmentTransaction.commit()
        }
    }
